...
class LifeExpectancyPredictor:
    ...
    def predict(self, data):
        ...
        age = float(data['age'])
        predicted_total = round(raw, 1)
        years_left = round(max(0, raw - age), 1)
        return predicted_total, years_left

    def factor_impacts(self, data):
        base_vec = _encode(data)
        base_s   = self.scaler.transform([base_vec])[0]
        base_pred= float(self.model.predict([base_s])[0])
        ...
        for key, (override, label, category) in tweaks.items():
            mod = {**data, **override}
            mv  = _encode(mod)
            ms  = self.scaler.transform([mv])[0]
            mp  = float(self.model.predict([ms])[0])
            delta = round(mp - base_pred, 1)
            if delta > 0.1:
                factors.append({
                    'label':    label,
                    'delta':    delta,
    ...
    def health_score(self, data):
        ...
        max_scores = {'Lifestyle': 40, 'Physical': 40, 'Mental': 20}

        scores['Lifestyle'] += {'never':3,'former':2,'light':1,'heavy':0}.get(data['smoking'],0) * 6
        scores['Lifestyle'] += {'none':3,'moderate':2,'heavy':0}.get(data['alcohol'],0) * 4
        scores['Lifestyle'] += {'none':0,'light':1,'moderate':2,'intense':3}.get(data['exercise'],0) * 5
        scores['Lifestyle'] += {'poor':0,'average':1,'good':2,'excellent':3}.get(data['diet'],0) * 4

        bmi = float(data['bmi'])
        scores['Physical'] += max(0, 10 - abs(bmi - 22) * 1.2)
        bp  = float(data['systolic_bp'])
        scores['Physical'] += max(0, 10 - max(0, bp - 120) * 0.25)
        chol= float(data['cholesterol'])
        scores['Physical'] += max(0, 10 - max(0, chol - 180) * 0.08)
        sugar=float(data['blood_sugar'])
        scores['Physical'] += max(0, 10 - max(0, sugar - 90) * 0.15)

        scores['Mental'] += {'low':3,'moderate':2,'high':1,'very_high':0}.get(data['stress'],0) * 3.5
        scores['Mental'] += {'less_5':0,'5_6':1,'7_8':3,'more_9':2}.get(data['sleep'],0) * 2.5

        total = sum(scores.values())
        max_t = sum(max_scores.values())
        pct   = round(total / max_t * 100)

        return {
            'total': pct,
            'breakdown': {k: round(v/max_scores[k]*100) for k,v in scores.items()},
        }
